...
export const getProductData = async (productId) => {
    try {
        const response = await axios.get(`${BASE_URL}product/${productId}.json`);
...
export const getProductsData = async (searchQuery) => {
    try {
        const response = await axios.get(`${BASE_URL}/products?search_terms=${searchQuery}&search_simple=1&action=process&json=1`);
        return response.data;
    } catch (error) {
        console.error(`Error: ${error}`); 
    }
